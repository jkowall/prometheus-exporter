...
// scrape serves whatever the healthy collectors returned and fails only when a
// collector failed and no supported collector succeeded.
func New(
	ctx context.Context,
	logger *zap.SugaredLogger,
	c client.Client,
...
// collector's health series are served, and the scrape fails only when a
// collector failed and no supported collector succeeded.
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, c := range e.collectors {
		...
		case errors.Is(err, ErrNotSupported):
			...
			log("Collector is not supported on this deployment", "collector", c.Name())
	...
	if len(issues) > 0 && (!e.partialScrapes || (failed > 0 && succeeded == 0)) {
